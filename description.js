...
seats.forEach((seatGroup) => {
  ...
  rows.innerHTML = `
    <div id="${seatGroup.price}" class="seat"> </div>
  `;
  rowPlace.appendChild(rows);

  const eachRow = document.getElementById(seatGroup.price);
  seatGroup.id.forEach((seatID) => {
    const place = document.createElement("div");
    place.innerHTML = `
    <button id="${seatID}" class="seat-button">${seatID}</button>
  `;  
